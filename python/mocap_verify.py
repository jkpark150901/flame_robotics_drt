...
mocap_verify.py
===============
Mocap-only 궤적 검증 툴 진입점.
로봇 SDK 없이 NatNet + 캘리브레이션 JSON + 계획 CSV만으로 동작.

실행:
  python python/mocap_verify.py
  python python/mocap_verify.py --config python/verifytool.cfg
"""
...
def main():
    parser = argparse.ArgumentParser(description="Mocap-only Trajectory Verify Tool")
    parser.add_argument('--config', default=str(_PYTHON_PATH / 'verifytool.cfg'),
    ...
    from verifytool.csv_mocap_verify import CsvMocapVerifyWindow
