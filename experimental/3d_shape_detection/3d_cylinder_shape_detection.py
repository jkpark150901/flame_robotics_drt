...
def main():
    ...
    if args.scale != 1.0:
        pcd.scale(args.scale, center=np.array([0.0, 0.0, 0.0]))
    ...
    for i, pt in enumerate(input_points):
        ...
        result = fit_cylinder(pcd, pt, height_limit=args.height, search_radius=args.search_radius)
        ...
        if result:
            ...
            mesh_cylinder = o3d.geometry.TriangleMesh.create_cylinder(
                radius=result["radius"], height=args.height
            )
